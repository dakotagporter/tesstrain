...
export LANG_MODEL='ssq'

# Iterate through all processed png files and change image dpi, segment images and generate each segment's corredsponding ground truth.
find . -type f -name "*.png_bw.png" -exec sh -c '
    for img_file do
        echo  "\r\n File: $img_file"
        mogrify -density 300 "$img_file"
        OMP_THREAD_LIMIT=1 tesseract --tessdata-dir ../tesseract/tessdata   "${img_file}" "${img_file%.*}"  --dpi 300 --psm 4  --oem 1 -l "${LANG_MODEL}" -c page_separator="" hocr
